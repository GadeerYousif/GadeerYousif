 ...
 * on-screen coordinates. Unlike the live camera, there's no sensor rotation
 * or crop to account for — the image is displayed at `displaySize`, which
 * the caller keeps at the same aspect ratio as `imageSize`, so this is a
 * plain uniform scale with no offset.
 */
